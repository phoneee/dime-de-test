...
from .schema_mappings import map_dividends_data
...
def fetch_and_process_delisted(api_key=None):
    if not api_key:
        api_key = API_KEY
    page_num = 0
    print(DELISTED_URL_TEMPLATE.format(page_num, api_key))
    while True:
...
def fetch_dividends(symbol, api_key=None):
    if not api_key:
        api_key=API_KEY
